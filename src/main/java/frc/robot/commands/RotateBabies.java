...
public class RotateBabies extends CommandBase {
  ...
  public void execute() {
    ...
    SmartDashboard.putNumber("Left Baby Position", leftPos);
    SmartDashboard.putNumber("Right Baby Position", rightPos);

    SmartDashboard.putBoolean("Left Baby < max", leftPos < Max);
    SmartDashboard.putBoolean("Right Baby < max", rightPos < Max);

    // Rotate Clockwise
    if (false && Buttons.J_BUTTON_FIVE.get() || rotatingClockwise) {
    ...
    else if (false && Buttons.J_BUTTON_SIX.get() || rotatingCounterClockwise) {
